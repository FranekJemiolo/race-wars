...
- Name: performance/leaderboard-performance.test.ts >> Leaderboard Performance Tests >> WebSocket Connection Performance >> should maintain connection stability under load
- Location: tests/performance/leaderboard-performance.test.ts:79:9
...
> 81  |       expect(activeConnections).toHaveLength(CONCURRENT_USERS);
  ...
  174 |     });
  175 | 
  176 |     test('should maintain update accuracy under load', async () => {
  177 |       // Test that position updates are processed correctly even under high load
  178 |       const testUpdates = 1000;
  179 |       const processedUpdates: number[] = [];
  180 |       
  181 |       // Monitor leaderboard updates
```