...
async function generateRaceReplaySystemScreenshot() {
  ...
  try {
    ...
    // Navigate to Race Replay
    console.log('Navigating to Race Replay...');
    const replayClicked = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button, [role="button"]'));
      for (const button of buttons) {
        const btnText = button.textContent || '';
        if (btnText.includes('🎬') || btnText.includes('Race Replay')) {
          button.click();
          return true;
        }
      }
    ...
    if (!replayClicked) {
      throw new Error('Could not click Race Replay button');
    }
...
generateRaceReplaySystemScreenshot();
