...
async function generateRouteBuilderScreenshot() {
  ...
  try {
    ...
    const routeClicked = await page.evaluate(() => {
      ...
      for (const button of buttons) {
        ...
        if (btnText.includes('🗺️') || btnText.includes('Route Builder')) {
          button.click();
          return true;
        }
      }
    ...
    if (!routeClicked) {
      throw new Error('Could not click Route Builder button');
    }
...
generateRouteBuilderScreenshot();
