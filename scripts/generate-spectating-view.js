...
async function generateSpectatingViewScreenshot() {
  ...
  try {
    ...
    console.log('Generating Spectating View screenshot...');
    ...
    // Navigate to spectating view by clicking a race's spectate button
    console.log('Navigating to Spectating View...');
    const spectateClicked = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button, [role="button"]'));
      for (const button of buttons) {
        const btnText = button.textContent || '';
        if (btnText.includes('Spectate') || btnText.includes('👁️')) {
          button.click();
          return true;
        }
      }
    ...
    if (!spectateClicked) {
      throw new Error('Could not click Spectate button');
    }
    ...
    await page.screenshot({ 
      path: 'docs/assets/spectating-view.png',
      fullPage: false 
    });
    console.log('✓ Spectating view screenshot generated');
...
generateSpectatingViewScreenshot();
