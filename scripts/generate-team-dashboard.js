...
async function generateTeamDashboardScreenshot() {
  ...
  try {
    ...
    const teamClicked = await page.evaluate(() => {
      ...
      for (const button of buttons) {
        ...
        if (btnText.includes('👥') || btnText.includes('Teams')) {
          button.click();
          return true;
        }
      }
    ...
    if (!teamClicked) {
      throw new Error('Could not click Teams button');
    }
...
generateTeamDashboardScreenshot();
